...
from core.v37_strategy import V37Strategy, V37Signal, get_v37_strategy
from core.kill_switch import KillSwitch

class V37LiveTrading:
    TARGET_TRADES = 30
    ...
    def __init__(self):
        ...
        self.strategy = get_v37_strategy()
        self.kill_switch = KillSwitch()
        self.data_dir = Path(__file__).parent / 'data'
        self.data_dir.mkdir(exist_ok=True)
        self.stats_file = self.data_dir / 'v37_stats.json'
        self.stats = {
            "version": "V3.7",
        ...
        self.running = False
        print("="*60, flush=True)
        print("🚀 V3.7 Exit 优化版启动", flush=True)
        print("="*60, flush=True)
        print("核心改变（只改 Exit）:", flush=True)
        print("  1. Trailing Stop: 回撤 0.15% 触发", flush=True)
        print("  2. STOP LOSS: -0.07%", flush=True)
        print("  3. TIME_EXIT: 90s", flush=True)
    ...
    def signal(self, c):
        ...
        return V37Signal(score=s, direction="LONG" if pc > 0 else "SHORT", volume_ratio=vr, price_change=pc)
    ...
    def check_entry(self):
        ...
        self.current_position = {
            ...
            "direction": d.trend_direction, "peak_price": c[-1]["close"]  # 关键：记录 peak
        }
    ...
    def check_exit(self):
        ...
        if ex:
            ...
            self.stats["trades"].append({"pnl": pnl, "exit_reason": ex, "entry_price": ep, "exit_price": c[-1]["close"]})
            self._save()
            emoji = "✅" if pnl > 0 else "❌"
            print(f"\n{emoji} 平仓: {ex} | PnL: {pnl*100:.4f}%", flush=True)
    ...
    def report(self):
        ...
        print(f"✅ 平均盈利: {aw*100:.4f}%", flush=True)
        print(f"❌ 平均亏损: {al*100:.4f}%", flush=True)
        print()
        print("Exit 分布:", flush=True)
        for k, v in self.stats["exit_distribution"].items():
            if v > 0: print(f"  {k}: {v}", flush=True)
        print("="*60, flush=True)
        
        if pf > 1.0: print("🟢 V3.7 有效！", flush=True)
        elif pf > 0.9: print("🟡 接近有效", flush=True)
        else: print("🔴 需要迭代", flush=True)
