...
验证 8 个关键点：
1. 请求签名是否成功
2. OKX 是否正常返回 ack/order id
3. venue 响应是否正确映射进本地 OrderStatus
4. protection 创建是否不受真实 venue 路径影响
5. event/correlation/causation 链是否不断
6. cockpit API 是否能读到真实 testnet 这笔单
7. admin freeze/cancel 是否能作用于真实 testnet 路径
8. 错误码、拒单、超时是否被正确归类
"""
...
class M2TestnetValidator:
    ...
    def __init__(self, config_path: Path):
        ...
        # 测试数据
        self.test_order_id = f"M2-TNET-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"
        self.test_symbol = "ETH-USDT-SWAP"
        self.test_qty = Decimal("0.01")
    ...
    def setup(self) -> bool:
        """设置测试环境"""
        print("=" * 60)
        print("🐉 M2 OKX Testnet 单笔验证")
        print("=" * 60)
        print()
        
        # 创建工作目录
        workspace = Path(__file__).parent.parent / "test_storage" / "m2_testnet"
    ...
    async def validate(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if balances:
                print(f"✅ 余额：USDT={balances.get('USDT', 0)}")
            else:
                print("⚠️  余额查询失败")
            ...
            if eth_ticker:
                results["details"]["eth_price"] = eth_ticker.get('last')
                print(f"✅ ETH 价格：{eth_ticker.get('last')} USDT")
            else:
                print("⚠️  ETH 行情获取失败")
            ...
            results["checks"]["event_chain"] = event_count >= 3
            ...
            results["checks"]["cockpit_readable"] = overview is not None
            results["details"]["cockpit"] = overview
            
            print(f"✅ Cockpit 数据可读")
    ...
    def print_summary(self, results: Dict[str, Any]) -> None:
        ...
        if results.get("passed"):
            print("🎉 M2 Testnet 验证通过！")
        else:
            print("⚠️  M2 Testnet 验证未完全通过")
...
async def main():
    ...
    # 创建验证器
    validator = M2TestnetValidator(config_path)
    ...
    # 保存报告
    report_path = Path(__file__).parent.parent / "reports" / "m2_testnet_validation.json"
