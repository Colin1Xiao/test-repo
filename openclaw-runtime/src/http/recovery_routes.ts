...
interface RecoveryState {
  ...
  validator: StateSequenceValidator;
  instanceId: string;
}
...
export function createRecoveryRoutes(
   ...
   * 续期 Session
   */
  router.post('/session/:id/renew', async (req: Request, res: Response) => {
    try {
  ...
  router.post('/session/:id/complete', async (req: Request, res: Response) => {
    try {
      ...
      const result = await state.coordinator.completeSession(
        session_id,
        { require_all_items_complete }
      );
   ...
   * 声明 Item 所有权
   */
  router.post('/items/:id/claim', async (req: Request, res: Response) => {
    try {
   ...
   * 完成 Item 处理
   */
  router.post('/items/:id/complete', async (req: Request, res: Response) => {
    try {
      const item_id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
      ...
      const result = await state.coordinator.completeItem(
        item_id,
        session_id,
        { success: item_success !== false, error: item_error }
      );
  ...
  router.post('/state/transition', async (req: Request, res: Response) => {
    try {
      ...
      // 创建临时状态对象（实际应从存储加载）
      const stateObject = state.validator.createStateObject(
        object_id,
        machine_id as any
      );
      
      const result = await state.validator.transition(
        stateObject,
        new_state,
        expected_version
      );
   ...
   * 列出所有状态机定义
   */
  router.get('/state/machines', (_req: Request, res: Response) => {
    res.json({
      success: true,
      data: {
        approvals: {
