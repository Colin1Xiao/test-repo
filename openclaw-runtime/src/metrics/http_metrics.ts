...
const httpRequestsSuccessTotal = registry.counter(
  'http_requests_success_total',
  'Successful HTTP requests',
  ['method', 'route']
);

const httpRequestsErrorTotal = registry.counter(
...
export function recordIdempotencyCreated(route: string): void {
  const counter = registry.counter('idempotency_created_total', 'Idempotency records created', ['route']);
  counter.inc({ route });
}

export function recordIdempotencyHit(route: string): void {
  const counter = registry.counter('idempotency_hit_total', 'Idempotency hits (duplicate requests)', ['route']);
  counter.inc({ route });
}

export function recordIdempotencyInProgress(route: string, delta: number): void {
...
export function recordLockAcquireSuccess(resource: string): void {
  const counter = registry.counter('lock_acquire_success_total', 'Successful lock acquisitions', ['resource']);
  counter.inc({ resource });
}

export function recordLockAcquireFailure(resource: string, reason: string): void {
...
export function recordRecoverySessionCompleted(): void {
...
export function recordRecoveryItemClaimSuccess(itemType: string): void {
...
export function recordRecoveryItemClaimFailure(itemType: string, reason: string): void {
...
export function recordStateTransitionAllowed(machine: string, from: string, to: string): void {
...
export function recordStateTransitionRejected(machine: string, from: string, to: string, reason: string): void {
...
export function recordRedisConnected(instance: string): void {
  const gauge = registry.gauge('redis_connected', 'Redis connection status (1=connected)', ['instance']);
  gauge.set(1, { instance });
}

export function recordRedisDisconnected(instance: string): void {
  const gauge = registry.gauge('redis_connected', 'Redis connection status (1=connected)', ['instance']);
  gauge.set(0, { instance });
}

export function recordAuditWriteSuccess(eventType: string): void {
...
export function recordAuditWriteFailure(eventType: string, reason: string): void {
