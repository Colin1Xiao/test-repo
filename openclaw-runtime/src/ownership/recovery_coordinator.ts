...
export interface RecoverySession {
  session_id: string;
  owner_id: string;           // 持有会话的实例 ID
  created_at: number;         // 创建时间戳 (ms)
  expires_at: number;         // 过期时间戳 (ms)
  status: 'active' | 'completed' | 'expired' | 'abandoned';
  items_total: number;        // 总 Item 数
  items_claimed: number;      // 已声明的 Item 数
  items_completed: number;    // 已完成的 Item 数
  last_heartbeat: number;     // 最后心跳时间 (ms)
}
...
export interface RecoveryItem {
  ...
  session_id?: string;        // 当前持有该 Item 的 Session ID
  claimed_at?: number;        // 声明时间 (ms)
  claimed_by?: string;        // 声明者 ID
  status: 'pending' | 'claimed' | 'completed' | 'failed' | 'abandoned';
  retry_count: number;
  max_retries: number;
  created_at: number;
  expires_at?: number;        // Item 过期时间 (ms)
}
...
export interface RecoveryCoordinatorConfig {
  session_ttl_ms: number;           // Session 默认 TTL (默认 30 分钟)
  session_heartbeat_interval_ms: number; // 心跳间隔 (默认 5 分钟)
  item_claim_ttl_ms: number;        // Item Claim TTL (默认 10 分钟)
  item_max_retries: number;         // 最大重试次数 (默认 3)
  orphan_detection_interval_ms: number; // 孤儿检测间隔 (默认 1 分钟)
}

export const DEFAULT_RECOVERY_CONFIG: RecoveryCoordinatorConfig = {
...
export class RecoveryCoordinator {
  ...
  // Redis key prefixes
  private static readonly SESSION_PREFIX = 'recovery:session:';
  private static readonly ITEM_PREFIX = 'recovery:item:';
  private static readonly SESSIONS_SET = 'recovery:sessions';
  ...
  async startSession(): Promise<SessionStartResult> {
    ...
    try {
      ...
      await this.redis.set(
        RecoveryCoordinator.SESSION_PREFIX + session_id,
        JSON.stringify(session),
        { ex: Math.ceil(this.config.session_ttl_ms / 1000) }
      );
   ...
   */
  async renewSession(session_id: string): Promise<SessionRenewResult> {
    const now = Date.now();
  ...
  async completeSession(
    session_id: string,
    options: { require_all_items_complete: boolean } = { require_all_items_complete: false }
  ...
  private async getActiveSession(): Promise<RecoverySession | null> {
  ...
  private async getSession(session_id: string): Promise<RecoverySession | null> {
    const data = await this.redis.get(RecoveryCoordinator.SESSION_PREFIX + session_id);
    if (!data) return null;
    return JSON.parse(data) as RecoverySession;
  }

  private async getItem(item_id: string): Promise<RecoveryItem | null> {
    const data = await this.redis.get(RecoveryCoordinator.ITEM_PREFIX + item_id);
    if (!data) return null;
    return JSON.parse(data) as RecoveryItem;
  }

  private async updateSession(session: RecoverySession): Promise<void> {
    await this.redis.set(
      RecoveryCoordinator.SESSION_PREFIX + session.session_id,
      JSON.stringify(session),
      { ex: Math.ceil((session.expires_at - Date.now()) / 1000) }
    );
  }

  private async updateItem(item: RecoveryItem): Promise<void> {
    const ttl = item.expires_at 
      ? Math.max(1, Math.ceil((item.expires_at - Date.now()) / 1000))
      : undefined;
    
    await this.redis.set(
      RecoveryCoordinator.ITEM_PREFIX + item.item_id,
      JSON.stringify(item),
  ...
  private async atomicClaimItem(
    ...
    const result = await this.redis.eval(
      luaScript,
      1,
      RecoveryCoordinator.ITEM_PREFIX + item_id,
      session_id,
      this.instance_id,
      String(now),
      String(this.config.item_claim_ttl_ms)
    );

    if (!result) return null;
    return JSON.parse(result) as RecoveryItem;
  }

  private async getIncompleteItems(_session_id: string): Promise<RecoveryItem[]> {
