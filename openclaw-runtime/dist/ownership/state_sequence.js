 ...
 */
export const APPROVALS_MACHINE = {
    id: 'approvals',
 ...
 */
export const INCIDENTS_MACHINE = {
    id: 'incidents',
    initial_state: 'open',
    terminal_states: ['resolved'],
    transitions: {
 ...
 */
export const DEPLOYMENTS_MACHINE = {
    id: 'deployments',
...
export class StateSequenceValidator {
    audit;
    machines;
    constructor(audit, customMachines) {
        this.audit = audit;
     ...
     * 验证状态转换是否合法
     */
    validateTransition(machine_id, from_state, to_state) {
     ...
     * 检查状态是否为终端状态
     */
    isTerminalState(machine_id, state) {
        const machine = this.machines[machine_id];
        if (!machine)
            return false;
     ...
     * 获取允许的下一个状态列表
     */
    getAllowedTransitions(machine_id, from_state) {
        const machine = this.machines[machine_id];
        if (!machine)
            return [];
     ...
     */
    async transition(stateObject, new_state, expected_version) {
        ...
        if (!transition.allowed) {
            const allowedTransitions = this.getAllowedTransitions(stateObject.machine_id, stateObject.current_state);
     ...
     * 创建新的状态对象
     */
    createStateObject(id, machine_id, metadata) {
        const machine = this.machines[machine_id];
        if (!machine) {
            throw new Error(`Unknown state machine: ${machine_id}`);
        }
 ...
 * 快速检查状态转换是否合法
 */
export function canTransition(machine_id, from, to) {
 ...
 * 获取所有允许的转换
 */
export function getAllowedTransitions(machine_id, from) {
    const machine = STATE_MACHINES[machine_id];
    if (!machine)
        return [];
 ...
 * 获取状态机的所有状态
 */
export function getAllStates(machine_id) {
    const machine = STATE_MACHINES[machine_id];
    if (!machine)
        return [];
...
//# sourceMappingURL=state_sequence.js.map