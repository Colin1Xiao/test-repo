...
This skill requires external process execution to render Mermaid diagrams using `mmdc` (Mermaid CLI). The implementation has been hardened with:

- **Parameter validation**: All arguments are validated to prevent shell injection
- **Path restriction**: Only relative paths within expected directories are allowed  
- **Fixed command**: Only executes `mmdc` with predefined safe arguments
- **No arbitrary code**: Cannot execute user-provided commands or scripts

The security audit may flag this as "dangerous-exec" because it uses `child_process.spawn`, but the actual risk is minimal since it only calls the trusted `mmdc` tool with validated arguments.
