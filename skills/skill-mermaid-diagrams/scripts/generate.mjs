...
import { safeMmdc } from "./safe-mmdc.mjs";
...
async function renderDiagramFromContent(diagramData, index) {
  ...
  const templatePath = path.join(TEMPLATES_DIR, `${template}.mmd.txt`);
...
async function renderDiagram({ mmdPath, templateName, index }) {
  ...
  try {
    ...
    await safeMmdc(['-i', mmdPath, '-o', svgPath, '-t', 'default', '-b', 'transparent'], {
      stdio: "ignore",
    });
    
    // Also render PNG for compatibility
    await safeMmdc(['-i', mmdPath, '-o', pngPath, '-t', 'default', '-b', 'transparent', '-w', '1200'], {
      stdio: "ignore",
    });
...
async function main() {
  try {
    ...
    try {
      ...
      spawnSync('which', ['mmdc'], { stdio: 'ignore' });
    } catch {
