...
## Session State Schema (MVP v1.0 - 已冻结)
...
```json
{
  ...
  "task": {
    ...
    "subtasks": [
      {
        "id": "t1",
        "title": "检查 launchd plist 配置",
        "status": "done"
      },
      {
        "id": "t2",
        "title": "检查 healthcheck.sh 是否有 cooldown",
        "status": "done"
      },
      {
        "id": "t3",
        "title": "添加状态文件记录上次告警时间",
        "status": "pending"
      }
    ],
    "constraints": [
  ...
  "context": {
    ...
    "assumptions": [
      {
        "text": "状态文件可以写在 ~/.openclaw/workspace/",
        "confidence": "high"
      }
    ],
    "openQuestions": [
  ...
  "artifacts": {
    ...
    "keyValues": {
      ...
      "last_alert": "2026-03-30T00:00:00Z"
    },
    "references": [
      {
        "id": "logs_tail_1",
        "type": "file_segment",
        "target": "logs/app.log",
        "locator": { "startLine": 1200, "endLine": 1260 },
        "summary": "最近一次报错堆栈"
      }
    ]
  },
...
| 字段 | 上限 | 清理规则 |
|------|------|---------|
| `context.confirmedFacts` | ≤20 | 去重，移除无来源 |
| `context.assumptions` | ≤10 | 移除 confidence=low 超过 3 轮 |
| `context.openQuestions` | ≤10 | 已回答的移除 |
| `progress.completed` | ≤20 | 保留最近 20 条 |
| `progress.attempted` | ≤10 | 每轮合并到 completed/failed |
| `progress.failed` | ≤10 | 保留最近 10 条 |
| `progress.pending` | ≤10 | 已完成的移除 |
| `artifacts.files` | ≤20 | 移除不存在的路径 |
| `artifacts.commands` | ≤10 | 保留最近 10 条 |
| `artifacts.keyValues` | ≤20 | 移除空值/过期值 |
| `artifacts.references` | ≤10 | 移除未引用的 |

### 事实/假设隔离规则

1. **confirmedFacts** 必须有明确来源：`user` / `tool_output` / `code_review` / `file_read`
2. **assumptions** 必须标注 confidence：`high` / `medium` / `low`
3. **禁止** 将推测写入 confirmedFacts
4. **禁止** 将无来源信息写入 confirmedFacts
