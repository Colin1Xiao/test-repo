...
动态标的管理系统 V2 (完全修复版)
每小时更新监控标的
OKX 合约市场交易量 TOP10 + 24 小时涨跌幅 TOP5
"""
            ...
            if response.status_code == 200:
                ...
                if data.get('code') == '0':
                    ...
                    for t in tickers:
                        ...
                        # 只处理 USDT 合约
                        if 'USDT' in inst_id:
                            # 使用 volCcy24h (USD 计价的交易额)
                            vol_ccy = t.get('volCcy24h', '0')
                            vol = float(vol_ccy) if vol_ccy else 0
                ...
                if data.get('code') == '0':
                    ...
                    for ticker in tickers:
                        ...
                        if 'USDT' in inst_id:
                            ...
                            if last > 0 and open_24h > 0 and vol > 10000:  # 过滤低流动性
    ...
    def update_symbols(self):
        ...
        if volume_data:
            ...
            for i, item in enumerate(volume_data[:5], 1):
                print(f"      {i}. {item['instId']}: ${item['volume']:,.0f} (@ ${item['price']:,.2f})")
            if len(volume_data) > 5:
                print(f"      ... 还有{len(volume_data)-5}个")
        else:
            print(f"   ❌ 获取失败，使用默认标的")
            volume_top10 = self.default_symbols.copy()
            self.volume_top10 = [{'symbol': s, 'volume': 0, 'instId': s, 'price': 0} for s in volume_top10]
        ...
        if change_data:
            ...
            print(f"   ✅ 获取成功 ({len(change_data)}个)")
            for item in change_data[:5]:
                arrow = "📈" if item['change'] > 0 else "📉"
                print(f"      {arrow} {item['instId']}: {item['change']:+.2f}% (${item['price']:,.2f}, Vol: ${item['volume']:,.0f})")
        else:
            print(f"   ❌ 获取失败，使用默认标的")
            change_top5 = []
            self.change_top5 = []
