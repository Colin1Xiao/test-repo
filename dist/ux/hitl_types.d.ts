 ...
 * 介入来源类型
 */
export type InterventionSourceType = 'task' | 'approval' | 'ops' | 'agent' | 'incident';
/**
 * 介入严重级别
 */
 ...
 * 介入状态
 */
export type InterventionStatus = 'open' | 'acknowledged' | 'in_review' | 'resolved' | 'dismissed' | 'snoozed' | 'escalated';
/**
 * 介入类型
 */
export type InterventionType = 'must_confirm' | 'should_review' | 'can_dismiss' | 'can_snooze' | 'should_escalate';
/**
 * 风险级别
 */
...
export interface InterventionRule {
    ...
    /** 生成介入项 */
    generateIntervention: (item: AttentionItem, dashboard: DashboardSnapshot) => InterventionItem;
}
/**
 * 介入引擎配置
 */
