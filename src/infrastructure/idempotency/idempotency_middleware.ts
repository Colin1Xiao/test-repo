...
import type { IdempotencyManager, IdempotencyKeyGenerator } from './idempotency_manager';
import type { DistributedLock, LockKeyGenerator } from '../lock/distributed_lock';
...
export interface IdempotencyMiddlewareConfig {
  ...
  lockGenerator?: LockKeyGenerator;
  idempotencyManager: IdempotencyManager;
  lock?: DistributedLock;
  lockTtlMs?: number;
  auditLog?: (event: IdempotencyAuditEvent) => Promise<void>;
}

export interface IdempotencyAuditEvent {
  type: 'idempotency_hit' | 'idempotency_created' | 'lock_acquired' | 'lock_acquire_failed' | 'lock_released' | 'lock_expired';
  key: string;
  ownerId?: string;
  timestamp: number;
...
export class IdempotencyMiddleware {
  private config: IdempotencyMiddlewareConfig;

  constructor(config: IdempotencyMiddlewareConfig) {
    this.config = config;
  }
   ...
   * 提取幂等键
   */
  extractIdempotencyKey(req: http.IncomingMessage, resourceType: string, resourceId: string): string {
  ...
  async begin(
    ...
    // 检查幂等性
    const beginResult = await this.config.idempotencyManager.begin(
      idempotencyKey,
      this.hashRequest(req)
    );
   ...
   * 完成幂等记录
   */
  async complete(context: IdempotencyContext, response: any): Promise<void> {
   ...
   * 失败幂等记录
   */
  async fail(context: IdempotencyContext, error: Error): Promise<void> {
   ...
   * 生成 Owner ID
   */
  private generateOwnerId(req: http.IncomingMessage): string {
    // 使用请求的唯一标识
   ...
   * 哈希请求
   */
  private hashRequest(req: http.IncomingMessage): string {
    // 简单实现，实际应该哈希请求体
   ...
   * 审计日志
   */
  private async audit(event: IdempotencyAuditEvent): Promise<void> {
...
export function withIdempotency(
  ...
  resourceType: string,
  resourceIdExtractor: (req: http.IncomingMessage) => string
): HttpHandler {
  return async (req: http.IncomingMessage, res: http.ServerResponse) => {
    try {
      const resourceId = resourceIdExtractor(req);
      
      const { context, shouldContinue, existingResponse } = await middleware.begin(
        req,
        resourceType,
        resourceId
      );
