...
export class TradingOperatorViews {
  private config: Required<TradingOperatorViewsConfig>;
  ...
  async buildDashboardSnapshot(
    ...
    deployments: any[],
    riskChanges: any[]
  ): Promise<TradingDashboardSnapshot> {
    const now = Date.now();
   ...
   */
  async buildActiveIncidents(alerts: any[]): Promise<TradingActiveIncidents> {
   ...
   */
  async buildPendingApprovals(approvals: any[]): Promise<TradingPendingApprovals> {
    ...
    const items = pendingApprovals.map((approval) => ({
      ...
      riskLevel: approval.metadata?.riskLevel as TradingSeverity,
   ...
   * 构建 Risk State 视图
   */
  async buildRiskState(riskChanges: any[]): Promise<TradingRiskState> {
    const now = Date.now();
    const currentRiskLevel = this.getCurrentRiskLevel(riskChanges);
   ...
   * 获取当前风险级别
   */
  private getCurrentRiskLevel(riskChanges: any[]): TradingSeverity {
    if (riskChanges.length === 0) {
      return 'low';
    }

    const lastChange = riskChanges[riskChanges.length - 1];
    return (lastChange.toLevel || lastChange.level || 'low') as TradingSeverity;
  }
}
...
export function createTradingOperatorViews(
  config?: TradingOperatorViewsConfig
): TradingOperatorViews {
