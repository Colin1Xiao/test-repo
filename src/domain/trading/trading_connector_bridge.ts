...
export class TradingConnectorBridge {
  private config: Required<TradingConnectorBridgeConfig>;
   ...
   */
  convertGitHubActionsDeployment(
    githubEvent: any
  ): TradingEvent | null {
    ...
    if (eventType === 'deployment' && deployment?.environment === 'production') {
      return {
        ...
        metadata: {
          ...
          ref: deployment.ref,
          riskLevel: 'high' as TradingSeverity,
        },
      };
    }
  ...
  convertSystemAlert(
    ...
    return {
      type: 'system_alert',
      timestamp: Date.now(),
      severity: severity as TradingSeverity,
      source: {
        system,
        component,
  ...
  convertRiskParameterChange(
    ...
    // 根据变化幅度判断风险级别
    let severity: TradingSeverity = 'medium';
    if (changeData.riskLevel) {
      severity = changeData.riskLevel as TradingSeverity;
...
export function createTradingConnectorBridge(
  config?: TradingConnectorBridgeConfig
): TradingConnectorBridge {
