...
import type {
  ...
  TradingReleaseReadiness,
  TradingActiveIncidents,
  TradingPendingApprovals,
  TradingRiskState,
  TradingSeverity,
} from './trading_types';
...
export class TradingDashboardProjection {
  private config: Required<TradingDashboardProjectionConfig>;
  ...
  async buildEnhancedDashboard(
    ...
    // 未确认的严重告警
    const unackCriticalAlerts = alerts.filter(
    ...
    // 待处理的高风险审批
    const pendingHighRiskApprovals = approvals.filter(
      (a) => a.status === 'pending' && a.metadata?.riskLevel === 'high'
    );
    ...
    return {
      ...
      risk: {
        level: riskState.level as TradingSeverity,
        lastChanged: riskState.lastChanged,
        recentChanges: riskState.recentChanges as Array<{ timestamp: number; from: TradingSeverity; to: TradingSeverity; reason: string; changedBy: string }>,
        breaches24h: riskState.breaches24h as Array<{ timestamp: number; metric: string; threshold: string; value: string }>,
   ...
   */
  async buildReleaseReadinessView(
    releaseId: string,
    release: any
  ): Promise<TradingReleaseReadiness & {
   ...
   * 构建 Incident 处理视图
   */
  async buildIncidentHandlingView(
    alerts: any[]
  ): Promise<TradingActiveIncidents & {
  ...
  private filterLast24h(items: any[], now: number): any[] {
  ...
  private async buildActiveIncidents(alerts: any[]): Promise<TradingActiveIncidents> {
    ...
    const items = activeAlerts.map((alert) => ({
      id: alert.id,
      type: alert.type,
      severity: alert.severity as TradingSeverity,
...
export function createTradingDashboardProjection(
  config?: TradingDashboardProjectionConfig
): TradingDashboardProjection {
